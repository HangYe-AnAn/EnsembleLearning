...
#' @export
#'
bagging_ridge_model <- function(X, y, n_bags = 100, K = NULL) {
  library(glmnet)
  if(is.null(K)){
  ...
  for (i in seq_along(indices_list)) {
    ...
    if (!is.null(top_predictors) && length(top_predictors) > 0) {
      ...
      fit_results <- fit_ridge_model(selected_data$X, boot_y)
...
fit_ridge_model <- function(X, y) {
  ...
  cv_model <- cv.glmnet(X_matrix, y_vector, alpha = 0, nfolds = 10)
  best_lambda <- cv_model$lambda.min

  # Fit the final model using the optimal lambda
  model <- glmnet(X_matrix, y_vector, alpha = 0, lambda = best_lambda)
...
#results <- bagging_ridge_model(X, y, n_bags = 100, K = 3)
#print(results)
