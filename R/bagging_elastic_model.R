...
#' \cr
#'
#' @return bagging_elastic_model returns an object of class "list". A list with:
...
bagging_elastic_model <- function(X, y, alpha = 0.5, n_bags = 100, K = NULL) {
  library(glmnet)
  if(is.null(K)){
  ...
  if(!is.numeric(alpha) && alpha > 0){
    stop("alpha have to be numeric and greater than zero.")
  }
  ...
  for (i in seq_along(indices_list)) {
    ...
    if (!is.null(top_predictors) && length(top_predictors) > 0) {
      ...
      fit_results <- fit_elastic_model(selected_data$X, boot_y, alpha = alpha)
...
fit_elastic_model <- function(X, y, alpha = 0.5) {
  require(glmnet)
  ...
  cv_model <- cv.glmnet(X_matrix, y_vector, alpha = alpha, nfolds = 10)
  best_lambda <- cv_model$lambda.min

  # Fit the final model using the optimal lambda
  model <- glmnet(X_matrix, y_vector, alpha = alpha, lambda = best_lambda)
...
#results <- bagging_elastic_model(X, y, n_bags = 100, K = 3)
#print(results)
